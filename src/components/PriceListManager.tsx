...
export const PriceListManager: React.FC<PriceListManagerProps> = ({
  ...
  const handleGoogleAuth = async () => {
    try {
      ...
      setSyncFeedback({
        success: false,
        message: `Error al autenticar con Google: ${err.message || 'Verifica permisos de ventanas emergentes'}`
      });
    } finally {
  ...
  const handleSyncGoogleSheet = async () => {
    ...
    try {
      ...
      const res = await fetchGoogleSheetsCatalog(
        sheetUrl.trim(),
        products,
        sheetTabName.trim() || undefined,
        token || undefined
      );
          ...
          {activeTab === 'sheets-guide' && (
            <div className="space-y-4">
              ...
              <div className="p-4 rounded-xl border border-zinc-200 bg-zinc-50 space-y-3">
                ...
                {/* Google Account OAuth Status / Connect Button */}
                {currentUser ? (
                  <div className="flex items-center justify-between p-2.5 bg-emerald-50 border border-emerald-200 rounded-lg text-xs">
                    <div className="flex items-center gap-2 text-emerald-900 font-medium">
                      ...
                      <span>Conectado con Google: <strong>{currentUser.email}</strong> (Acceso a Sheets activo)</span>
                    </div>
                    <button
                      type="button"
                      onClick={handleGoogleAuth}
                      disabled={isAuthLoading}
                      className="text-[11px] font-semibold text-emerald-700 hover:text-emerald-950 underline cursor-pointer ml-2 shrink-0"
                    >
                      Cerrar sesión
                    </button>
                  </div>
                ) : (
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 p-2.5 bg-white border border-zinc-200 rounded-lg text-xs">
                    <div className="flex items-center gap-2 text-zinc-700">
                      ...
                      <span>¿Tu hoja es privada? Inicia sesión con Google para sincronizarla directamente sin hacerla pública.</span>
                    </div>
                    <button
                      type="button"
                      onClick={handleGoogleAuth}
                      disabled={isAuthLoading}
                      className="px-3 py-1.5 bg-[#181818] hover:bg-black text-white text-[11px] font-bold rounded-md flex items-center justify-center gap-1.5 cursor-pointer shrink-0 transition-colors shadow-xs"
                    >
                      <span>{isAuthLoading ? 'Conectando...' : 'Conectar con Google'}</span>
                    </button>
                  </div>
                )}
